...
void Schema::ProjectSchema(Schema s, vector<string> names, vector<int> &attrsToKeep) {
    numAtts = names.size();
    ...
    for (int i = 0; i < numAtts; i++) {
        attrsToKeep.push_back (s.Find(strdup(names[i].c_str())));
        myAtts[i] = s.myAtts[attrsToKeep.back()];
    }
}
