...
OrderMaker::OrderMaker(Schema *schema) {
    ...
    for (int counter = 0; counter < size; counter++) {
        if (atts[counter].myType == Int) {
            whichAtts[numAtts] = counter;
...
void OrderMaker::Print() {
    ...
    for (int counter = 0; counter< numAtts; counter++) {
        printf("%3d: %5d ", counter, whichAtts[counter]);
        if (whichTypes[counter] == Int)
            printf("Int\n");
        else if (whichTypes[counter] == Double)
            printf("Double\n");
        else
            printf("String\n");
    }
...
void OrderMaker::GrowFromParseTree(NameList *nameList, Schema *inputSchema) {
    for (; nameList; nameList = nameList->next, numAtts++) {
        whichAtts[numAtts] = inputSchema->Find(nameList->name);
        whichTypes[numAtts] = inputSchema->FindType(nameList->name);
    }
}
...
void CNF::GrowFromParseTree(struct AndList *parseTree, Schema *leftSchema, Schema *rightSchema, Record &literal) {
    ...
    FILE *outRecFile = fopen ("mnartsfFODWER", "w");
    ...
    FILE *outSchemaFile = fopen ("uiopqwerZXCBNM", "w");
    ...
    outRecFile = fopen ("mnartsfFODWER", "r");

    // read in the record's schema
    Schema mySchema("uiopqwerZXCBNM", "tempSchema");
    ...
    remove("mnartsfFODWER");
    remove("uiopqwerZXCBNM");
}
...
void CNF::GrowFromParseTree(struct AndList *parseTree, Schema *mySchema, Record &literal) {
    ...
    // read in the record's schema
    Schema outSchema("uiopqwerZXCBNM", "tempSchema");
