...
Record::Record(const Record &rec) : bits(NULL) {
    __builtin_prefetch (rec.bits);
    if (rec.bits == NULL) {
        return;
    }
...
Record & Record :: operator = (Record const &rec) {
    if (this != &rec) {
        delete [] bits;
        bits = NULL;
        if(NULL != rec.bits){
...
void Record::Print(Schema *schema, std::ostream &os) {
    Attribute *attr = schema->GetAtts();
    int attrCount = schema->GetNumAtts();

    for (int i = 0; i < attrCount; i++) {
        os << attr[i].name << ": [";
        int ptr = ((int *) bits)[i + 1];
        if (attr[i].myType == Int) {
            int *temp = (int *) &(bits[ptr]);
            os << *temp;
        } else if (attr[i].myType == Double) {
            double *tempDbl = (double *) &(bits[ptr]);
            os << *tempDbl;
        } else if (attr[i].myType == String) {
